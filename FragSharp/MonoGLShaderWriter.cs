...
namespace FragSharp
{
    ...
    /// </summary>
    internal class MonoGLShaderWriter : MonoGLWriter
    {
        public MonoGLShaderWriter(Dictionary<SyntaxTree, SemanticModel> models, Compilation compilation, Dictionary<Symbol, string> specialization)
...
VertexToPixel StandardVertexShader(vec2 inPos : POSITION0, vec2 inTexCoords : TEXCOORD0, vec4 inColor : COLOR0)
{{";
...
{{
{0}vec4 Position   : POSITION0;
{0}vec4 Color      : COLOR0;
{0}vec2 TexCoords  : TEXCOORD0;
{0}vec2 Position2D : TEXCOORD2;
}};
